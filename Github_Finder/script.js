...
async function fetchRepositories(repoUrl) {
  ...
  try {
    const response = await fetch(repoUrl + "?per_page=20");
...
function displayRepos(repos) {
  ...
  repos.forEach(repo => {
    ...
    const updateAt = formatDate(repo.update_at);
...
function displayUserData(user) {
  ...
  if (user.blog) {
    ...
    blogElement.href = user.blog.startWith("http") ? user.blog : `https://${user.blog}`;
  } else {
...
searchInput.value = "fahrypratama17";
searchUser();